...
public class BookRankCacheManager {
     ...
     * 查询小说点击榜列表，并放入缓存中
     *
     * 只有点击榜放入了Redis中，其他两个放的是Caffeine
     */
    @Cacheable(cacheManager = CacheConsts.REDIS_CACHE_MANAGER,
        value = CacheConsts.BOOK_VISIT_RANK_CACHE_NAME)
    public List<BookRankRespDto> listVisitRankBooks() {
        QueryWrapper<BookInfo> bookInfoQueryWrapper = new QueryWrapper<>();
        bookInfoQueryWrapper.orderByDesc(DatabaseConsts.BookTable.COLUMN_VISIT_COUNT); // 按照访问次数降序排列
    ...
    private List<BookRankRespDto> listRankBooks(QueryWrapper<BookInfo> bookInfoQueryWrapper) {
        bookInfoQueryWrapper
            .gt(DatabaseConsts.BookTable.COLUMN_WORD_COUNT, 0) // 保证有词
            .last(DatabaseConsts.SqlEnum.LIMIT_30.getSql()); // 取30条
